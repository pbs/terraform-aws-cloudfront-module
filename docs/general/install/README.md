...
```hcl
module "cloudfront" {
    source = "github.com/pbs/terraform-cloudfront-module?ref=x.y.z"
}
```
...
gh -R 'pbs/terraform-cloudfront-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/cloudfront'
rm -f release.tar.gz
```
...
```bash
git remote add -f cloudfront git@github.com:pbs/terraform-cloudfront-module.git
git subtree add --prefix terraform/modules/cloudfront cloudfront main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/cloudfront cloudfront $REF --squash
```
